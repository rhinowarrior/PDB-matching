...
def main():
    ...
    for key, value in model_dict.items():
        ...
        reference = str(Path(reference_dir, key + "_renumb.pdb"))
        print(f"Reference file: {reference}")

        for val in value:
            # Create the path to the decoy model
            val_path = str(Path(pipeline_dir, key, "merged", val))
            # Maps the reference and decoy PDBs, then saves them with the following naming format {key}_reference_{merged_number}_mapped.pdb and {key}_merged_{merged_number}_mapped.pdb
            pdb_ref, pdb_decoy = map_and_save_pdbs(reference, val_path, output_dir, key)
...
def map_and_save_pdbs(reference, decoy, output_dir, key):
    ...
    Args:
        ...
        key (str): Key to identify the model
    ...
    ref_outfile = str(Path(output_dir, f"{key}_reference_{merged_number}_mapped.pdb"))
    decoy_outfile = str(Path(output_dir, f"{key}_merged_{merged_number}_mapped.pdb"))
    
    write_PDB(pdb_ref, ref_outfile)
    write_PDB(pdb_decoy, decoy_outfile)
    print(f"Saved mapped PDBs for {key}: {ref_outfile}, {decoy_outfile}")
...
def map_PDBs(pdb_ref, pdb_decoy, remove_non_mapped=True, custom_map={}):
    ...
    # Print the residues that are removed from the PDBs
